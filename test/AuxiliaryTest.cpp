...
TEST_CASE( "deref_cast_test", "[auxiliary]" )
{
    ...
    REQUIRE_THROWS_AS(
        deref_dynamic_cast< C const >( ptr ), std::runtime_error );
...
TEST_CASE( "string_test", "[auxiliary]" )
{
    ...
    REQUIRE(
        "strinGstringstring" ==
        replace_first( "stringstringstring", "g", "G" ) );
    REQUIRE(
        "#stringstring" ==
        replace_first( "stringstringstring", "string", "#" ) );

    REQUIRE(
        "stringstringstrinG" ==
        replace_last( "stringstringstring", "g", "G" ) );
    REQUIRE(
        "stringstring#" ==
        replace_last( "stringstringstring", "string", "#" ) );

    REQUIRE(
        "/normal/path" == replace_all( "////normal//////path", "//", "/" ) );
...
namespace test
{
    struct Dotty : public TestHelper
    {
        ...
        int att1() const
        {
            return variantSrc::get< int >(
                getAttribute( "att1" ).getResource() );
        }
        double att2() const
        {
            return variantSrc::get< double >(
                getAttribute( "att2" ).getResource() );
        }
