    ...
    for( int step = 0; step <= max_step; step += 20 )
    {
        ...
        if( rank != 0 || declareFromAll )
        {
            ...
            e[ "positionOffset" ][ "x" ].resetDataset( Dataset(
                determineDatatype( positionOffset_local ), { num_cells } ) );
            ...
            {
                e[ "position" ][ "x" ].storeChunk(
                    position_local, { offset }, { rank } );
...
TEST_CASE( "hdf5_write_test", "[parallel][hdf5]" )
{
    ...
    Series o = Series(
        "../samples/parallel_write.h5", Access::CREATE, MPI_COMM_WORLD );
    ...
    *positionOffset_local = positionOffset_global[ mpi_rank ];
    ...
    e[ "positionOffset" ][ "x" ].storeChunk(
        positionOffset_local, { mpi_rank }, { 1 } );
...
{
    Series o = Series(
        "../samples/parallel_write.bp", Access::CREATE, MPI_COMM_WORLD );
...
void file_based_write_read( std::string file_ending )
{
...

    {
        ...
        for( int step = 0; step < last_step; ++step )
        {
            ...
            E_x.storeChunk(
                io::shareRaw( E_x_data ), chunk_offset, chunk_extent );
            series.flush();
        }
    }
...
void hipace_like_write( std::string file_ending )
{
         ...
         ++first_rank_step )
    {
        ...
        bool const start_new_output_step =
            std::find(
                iterations.begin(), iterations.end(), first_rank_step ) !=
            iterations.end();
        ...
        auto const copyToShared = []( std::vector< precision > const & data ) {
            auto d = std::shared_ptr< precision >(
                new precision[ data.size() ],
                std::default_delete< precision[] >() );
    ...
    std::string writeConfigBP4 = R"END(
{
  "adios2": {
    ...
    auto const write =
        [ size,
          rank ]( std::string const & filename, std::string const & config ) {
