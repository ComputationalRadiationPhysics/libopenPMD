...
TEST_CASE( "versions_test", "[core]" )
{
    ...
    REQUIRE(
        2u ==
        std::count_if(
            apiVersion.begin(), apiVersion.end(), []( char const c ) {
                return c == '.';
            } ) );
...
TEST_CASE( "output_constructor_test", "[core]" )
{
    ...
    o.setMeshesPath( "customMeshesPath" )
        .setParticlesPath( "customParticlesPath" );
...
TEST_CASE( "record_modification_test", "[core]" )
{
    ...
    std::array< double, 7 > e_field_unitDimension{
        { 1., 1., -3., -1., 0., 0., 0. } };
    REQUIRE( r.unitDimension() == e_field_unitDimension );

    r.setUnitDimension( { { RUD::L, 0. }, { RUD::T, -2. } } );
    std::array< double, 7 > b_field_unitDimension{
        { 0., 1., -2., -1., 0., 0., 0. } };
...
TEST_CASE( "structure_test", "[core]" )
{
    ...
    REQUIRE( o.iterations[ 1 ]
                 .meshes[ "M2" ][ MeshRecordComponent::SCALAR ]
                 .IOHandler() );
    ...
    REQUIRE(
        o.iterations[ 1 ]
            .meshes[ "M2" ][ MeshRecordComponent::SCALAR ]
            .parent() == getWritable( &o.iterations[ 1 ].meshes ) );
    scalar_mrc = o.iterations[ 1 ]
                     .meshes[ "M2" ][ MeshRecordComponent::SCALAR ]
                     .makeConstant( 1.0 );
    REQUIRE( scalar_mrc.IOHandler() );
    REQUIRE( o.iterations[ 1 ].meshes[ "M2" ].IOHandler() );
    ...
    REQUIRE(
        ...
        getWritable( &o.iterations[ 1 ].particles[ "P" ][ "PR" ] ) );
    rc =
        o.iterations[ 1 ].particles[ "P" ][ "PR" ][ "PRC" ].makeConstant( 1.0 );
    REQUIRE( rc.IOHandler() );
    REQUIRE( o.iterations[ 1 ].particles[ "P" ][ "PR" ][ "PRC" ].IOHandler() );
    REQUIRE(
        rc.parent() ==
    ...
    REQUIRE( o.iterations[ 1 ].particles[ "P" ].particlePatches.IOHandler() );
    REQUIRE(
        pp.parent() == getWritable( &o.iterations[ 1 ].particles[ "P" ] ) );
    REQUIRE(
        o.iterations[ 1 ].particles[ "P" ].particlePatches.parent() ==
        getWritable( &o.iterations[ 1 ].particles[ "P" ] ) );
    ...
    REQUIRE(
        prc.parent() ==
        getWritable(
            &o.iterations[ 1 ].particles[ "P" ].particlePatches[ "extent" ] ) );
    REQUIRE(
        o.iterations[ 1 ]
            .particles[ "P" ]
            .particlePatches[ "extent" ][ "x" ]
...
TEST_CASE( "wrapper_test", "[core]" )
{
    ...
    REQUIRE(
        o.iterations[ 1 ].meshes[ "E" ][ "x" ].getExtent() == Extent{ 7 } );

    Container< Iteration, uint64_t > its = o.iterations;
    its[ 1 ].meshes[ "E" ][ "y" ].resetDataset(
        Dataset( Datatype::CHAR, { 2 } ) );
    REQUIRE( o.iterations[ 1 ].meshes[ "E" ].count( "y" ) == 1 );
    REQUIRE(
        o.iterations[ 1 ].meshes[ "E" ][ "y" ].getDatatype() ==
        Datatype::CHAR );
    REQUIRE(
        o.iterations[ 1 ].meshes[ "E" ][ "y" ].getExtent() == Extent{ 2 } );
    ...
    o.iterations[ 4 ].meshes[ "E" ][ "y" ].loadChunk(
        shareRaw( moreData ), { 0 }, { 1 } );
    o.flush();
    REQUIRE( moreData[ 0 ] == value );
    auto all_data =
        o.iterations[ 4 ].meshes[ "E" ][ "y" ].loadChunk< double >();
    o.flush();
    ...
    o.iterations[ 6 ]
        ...
        .resetDataset( dset );
    ParticlePatches pp =
        o.iterations[ 6 ].particles[ "electrons" ].particlePatches;
    REQUIRE(
        pp[ "numParticles" ][ RecordComponent::SCALAR ].getDatatype() ==
        determineDatatype< uint64_t >() );
    REQUIRE(
        pp[ "numParticles" ][ RecordComponent::SCALAR ].getExtent() ==
        Extent{ 4 } );
    ...
    REQUIRE( o.iterations[ 6 ]
                 ...
                 .m_chunks->empty() );
    REQUIRE(
        pp[ "numParticles" ][ RecordComponent::SCALAR ].m_chunks->empty() );
#endif
    ...
    REQUIRE(
        o.iterations[ 6 ]
            ...
            .m_chunks->size() == 1 );
    REQUIRE(
        pp[ "numParticles" ][ RecordComponent::SCALAR ].m_chunks->size() == 1 );
#endif
    ...
    REQUIRE(
        o.iterations[ 6 ]
            ...
            .m_chunks->size() == 2 );
    REQUIRE(
        pp[ "numParticles" ][ RecordComponent::SCALAR ].m_chunks->size() == 2 );
#endif
    o.flush();
#if openPMD_USE_INVASIVE_TESTS
    REQUIRE( o.iterations[ 6 ]
