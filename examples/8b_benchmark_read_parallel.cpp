...
public:
    ...
    void read( std::string & filename )
    {
        try
        {
            ...
            Series series =
                Series( filename, Access::READ_ONLY, MPI_COMM_WORLD );
    ...
    void fullscan( Series & series, MeshRecordComponent & rho )
    {
        ...
        if( ( meshExtent[ 0 ] % grid[ 0 ] != 0 ) ||
            ( meshExtent[ 1 ] % grid[ 1 ] != 0 ) )
        {
            if( 0 == m_MPIRank )
                std::cerr
    ...
    void block( Series & series, MeshRecordComponent & rho )
    {
        ...
        bool atBottomRight =
            ( ( m_Pattern % 10 == 2 ) && ( fractionOnDim > 1 ) );
        ...
        for( unsigned int i = 0; i < meshExtent.size(); i++ )
        {
            ...
            else
            {
                ...
                if( !diagnalBlocks )
                    if( i != alongDim )
                        off[ i ] =
                            ( fractionOnDim / 2 ) * blob; // middle corner
            }
        }
     ...
     * @param series     openPMD series
     */
    void sliceField( Series & series, int ts )
    {
        if( m_Pattern >= 100 )
            return;
     ...
     *
     */
    void readStep( Series & series, int ts )
    {
        std::string comp_name = openPMD::MeshRecordComponent::SCALAR;

        MeshRecordComponent rho =
            series.iterations[ ts ].meshes[ "rho" ][ comp_name ];
        ...
        if( m_Pattern == 7 )
        {
            ...
            RecordComponent charge =
                electrons[ "charge" ][ RecordComponent::SCALAR ];
