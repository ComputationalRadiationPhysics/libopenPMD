...
void AbstractPattern::store( Series & series, int step )
{
    ...
    ParticleSpecies & currSpecies =
        series.iterations[ step ].particles[ "ion" ];
 ...
 *
 */
void AbstractPattern::storeParticles(
    ParticleSpecies & currSpecies, int & step )
{
    ...
    currSpecies[ "charge" ][ RecordComponent::SCALAR ].resetDataset(
        realDataSet );
 ...
 */
OneDimPattern::OneDimPattern( const TestInput & input )
 ...
 * @param count:  return
 */
unsigned long AbstractPattern::getNthMeshExtent(
    unsigned int n, Offset & offset, Extent & count )
{
    if( n > m_InRankMeshLayout.size() )
        return 0;
 ...
 * @param result: return
 */
inline void TwoDimPattern::coordinate(
...
{
    {
        ...
        m_GlobalUnitMesh = {
            input.m_XFactor, input.m_YFactor, input.m_ZFactor };

        PrintMe();
    }

    // unsigned long zFactor = input.m_YFactor;
    auto m = ( input.m_ZFactor * input.m_XFactor * input.m_YFactor ) %
        input.m_MPISize;
    if( m != 0 )
        throw std::runtime_error(
            "Unable to balance load for 3D mesh among ranks " );

    m = ( input.m_ZFactor * input.m_XFactor * input.m_YFactor ) /
        input.m_MPISize;
    auto maxRatio = input.m_MaxOverMin[ 0 ] * input.m_MaxOverMin[ 1 ] *
    ...
    else
    {
            ...
            ( ( input.m_XFactor * input.m_YFactor ) % input.m_MPISize == 0 ) )
        {
            if( input.m_XFactor % m == 0 )
                ...
                throw std::runtime_error(
                    "Wait for next version with other 3D patch "
                    "configurations" );
        }
    }
}
