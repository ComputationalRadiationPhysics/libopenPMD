...
inline constexpr Datatype determineDatatype()
{
    ...
    else if( decay_equiv< T, std::vector< std::complex< long double > > >::
                 value )
    {
...
inline bool isSame( openPMD::Datatype const d, openPMD::Datatype const e )
{
    ...
    if( d_is_fp && e_is_fp && d_is_vec == e_is_vec &&
        toBits( d ) == toBits( e ) )
        return true;
    ...
    if( d_is_cfp && e_is_cfp && d_is_vec == e_is_vec &&
        toBits( d ) == toBits( e ) )
        return true;
...
void warnWrongDtype(
    std::string const & key, Datatype store, Datatype request );
