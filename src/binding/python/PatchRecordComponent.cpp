        ...
        .def(
            ...
            []( PatchRecordComponent & prc ) {
                ...
                if( prc.getDatatype() == Datatype::CHAR )
                    ...
                    prc.load< double >(
                        shareRaw( ( double * )a.mutable_data() ) );
                else if( prc.getDatatype() == Datatype::FLOAT )
                    prc.load< float >(
                        shareRaw( ( float * )a.mutable_data() ) );
        ...
        .def(
            ...
            []( PatchRecordComponent & prc, uint64_t idx, py::buffer a ) {
                ...
                if( numElements == 1 )
                {
                    ...
                    case DT::BOOL:
                        return prc.store(
                            idx, *static_cast< bool * >( buf.ptr ) );
                        break;
                    case DT::SHORT:
                        return prc.store(
                            idx, *static_cast< short * >( buf.ptr ) );
                        break;
                    case DT::INT:
                        return prc.store(
                            idx, *static_cast< int * >( buf.ptr ) );
                        break;
                    case DT::LONG:
                        return prc.store(
                            idx, *static_cast< long * >( buf.ptr ) );
                        break;
                    case DT::LONGLONG:
                        return prc.store(
                    ...
                    case DT::FLOAT:
                        return prc.store(
                            idx, *static_cast< float * >( buf.ptr ) );
                        break;
                    case DT::DOUBLE:
                        return prc.store(
                            idx, *static_cast< double * >( buf.ptr ) );
                        break;
                    case DT::LONG_DOUBLE:
                        return prc.store(
