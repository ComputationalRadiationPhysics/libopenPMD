...
namespace
{
     ...
     */
    auxiliary::Option< WrittenChunkInfo > mergeChunks(
        WrittenChunkInfo const & chunk1, WrittenChunkInfo const & chunk2 )
    {
        /*
         * Idea:
    ...
    Writable * writable, Parameter< Operation::OPEN_DATASET > & parameters )
{
    ...
    *parameters.dtype = Datatype(
        stringToDatatype( datasetJson[ "datatype" ].get< std::string >() ) );
    ...
    Writable * writable, Parameter< Operation::WRITE_ATT > const & parameter )
{
    ...
    ( *jsonVal )[ filePosition->id ][ "attributes" ][ parameter.name ] = {
        { "datatype", datatypeToString( parameter.dtype ) },
        { "value", value } };
    writable->written = true;
    m_dirty.emplace( file );
}
...
JSONIOHandlerImpl::getPossiblyExisting( std::string file )
{
    ...
    auto it = std::find_if(
        ...
        m_files.end(),
        [ file ](
            std::unordered_map< Writable *, File >::value_type const & entry ) {
...
T JSONIOHandlerImpl::JsonToCpp< T, Dummy >::operator()(
...
template< typename T, int n >
std::array< T, n >
JSONIOHandlerImpl::JsonToCpp< std::array< T, n > >::operator()(
    nlohmann::json const & json )
{
    std::array< T, n > a;
    JsonToCpp< T > jtp;
