...
hid_t openPMD::GetH5DataType::operator()( Attribute const & att )
{
    ...
    case DT::VEC_CFLOAT:
        return H5Tcopy(
            m_userTypes.at( typeid( std::complex< float > ).name() ) );
    case DT::CDOUBLE:
    case DT::VEC_CDOUBLE:
        return H5Tcopy(
            m_userTypes.at( typeid( std::complex< double > ).name() ) );
...
hid_t openPMD::getH5DataSpace( Attribute const & att )
{
    ...
    case DT::VEC_UCHAR: {
        hid_t vec_t_id = H5Screate( H5S_SIMPLE );
        hsize_t dims[ 1 ] = {
            att.get< std::vector< unsigned char > >().size() };
        H5Sset_extent_simple( vec_t_id, 1, dims, nullptr );
        return vec_t_id;
    }
    case DT::VEC_USHORT: {
        hid_t vec_t_id = H5Screate( H5S_SIMPLE );
        hsize_t dims[ 1 ] = {
            att.get< std::vector< unsigned short > >().size() };
        H5Sset_extent_simple( vec_t_id, 1, dims, nullptr );
        return vec_t_id;
    }
    case DT::VEC_UINT: {
    ...
    case DT::VEC_ULONG: {
        hid_t vec_t_id = H5Screate( H5S_SIMPLE );
        hsize_t dims[ 1 ] = {
            att.get< std::vector< unsigned long > >().size() };
        H5Sset_extent_simple( vec_t_id, 1, dims, nullptr );
        return vec_t_id;
    }
    case DT::VEC_ULONGLONG: {
        hid_t vec_t_id = H5Screate( H5S_SIMPLE );
        hsize_t dims[ 1 ] = {
            att.get< std::vector< unsigned long long > >().size() };
        H5Sset_extent_simple( vec_t_id, 1, dims, nullptr );
        return vec_t_id;
    }
    case DT::VEC_FLOAT: {
