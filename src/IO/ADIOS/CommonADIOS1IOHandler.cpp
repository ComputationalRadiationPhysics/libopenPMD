    ...
    Parameter< Operation::CREATE_DATASET > const & parameters )
{
    ...
    if( !writable->written )
    {
        ...
        for( size_t i = 0; i < ndims; ++i )
        {
            ...
            chunkOffset[ i ] =
                "/tmp" + path + "_chunkOffset" + std::to_string( i );
            id = adios_define_var(
                group,
                chunkOffset[ i ].c_str(),
                "",
    ...
    Writable * writable, Parameter< Operation::OPEN_FILE > const & parameters )
{
    ...
    auto it = std::find_if(
        ...
        m_filePaths.end(),
        [ name ](
            std::unordered_map< Writable *, std::shared_ptr< std::string > >::
    ...
    if( m_openWriteFileHandles.find( filePath ) !=
    ...
    Parameter< Operation::DELETE_FILE > const & parameters )
{
    ...
    if( writable->written )
    {
        ...
        if( m_openWriteFileHandles.find( path ) !=
            m_openWriteFileHandles.end() )
        {
    ...
    Parameter< Operation::WRITE_DATASET > const & parameters )
{
    ...
    for( size_t i = 0; i < ndims; ++i )
    {
        ...
        status =
            adios_write( fd, chunkOffset.c_str(), &parameters.offset[ i ] );
        VERIFY(
            status == err_no_error,
            "[ADIOS1] Internal error: Failed to write ADIOS variable during "
            "Dataset writing" );
    ...
    Writable * writable, Parameter< Operation::READ_ATT > & parameters )
{
    ...
    VERIFY(
        ...
        "attribute read" );
    VERIFY(
        size != 0, "[ADIOS1] Internal error: ADIOS1 read 0-size attribute" );
    ...
    else
    {
        ...
        case adios_short: {
            if( sizeof( short ) == 2u )
                ...
                std::tie( a, dtype ) = std::make_tuple(
                    readVectorAttributeInternal< int >( data, size ),
                    DT::VEC_INT );
            else if( sizeof( long ) == 2u )
                std::tie( a, dtype ) = std::make_tuple(
        ...
        case adios_unsigned_short: {
            if( sizeof( unsigned short ) == 2u )
                ...
                std::tie( a, dtype ) = std::make_tuple(
                    readVectorAttributeInternal< unsigned long long >(
                        data, size ),
                    DT::VEC_ULONGLONG );
            else
                throw unsupported_data_error(
    ...
    else
    {
        ...
        for( int i = 0; i < f->nattrs; ++i )
        {
            ...
            if( auxiliary::starts_with( s, name ) )
            {
                /* remove the writable's path from the name */
                s = auxiliary::replace_first( s, name, "" );
                if( std::none_of( s.begin(), s.end(), []( char c ) {
                        return c == '/';
                    } ) )
                {
