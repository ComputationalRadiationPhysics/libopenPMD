...
SeriesImpl::SeriesImpl(
    ...
    : AttributableImpl{ attri }, m_series{ series }
{
}

std::string SeriesImpl::openPMD() const
{
...
std::string SeriesImpl::iterationFormat() const
{
    return getAttribute( "iterationFormat" ).get< std::string >();
}

SeriesImpl & SeriesImpl::setIterationFormat( std::string const & i )
{
    if( written() )
        throw std::runtime_error(
...
std::string SeriesImpl::name() const
{
    return get().m_name;
}

SeriesImpl & SeriesImpl::setName( std::string const & n )
{
    auto & series = get();
    if( written() )
        throw std::runtime_error(
            "A files name can not (yet) be changed after it has been "
            "written." );
...
void SeriesImpl::initDefaults( IterationEncoding ie )
{
...
std::future< void > SeriesImpl::flush_impl(
...
void SeriesImpl::flushFileBased(
...
void SeriesImpl::flushGorVBased(
    iterations_iterator begin, iterations_iterator end )
{
    auto & series = get();
    if( IOHandler()->m_frontendAccess == Access::READ_ONLY )
...
void SeriesImpl::flushMeshesPath()
{
...
void SeriesImpl::flushParticlesPath()
{
    Parameter< Operation::WRITE_ATT > aWrite;
...
void SeriesImpl::readFileBased()
{
    ...
    for( auto const & entry :
         auxiliary::list_directory( IOHandler()->directory ) )
    {
    ...
    if( series.iterations.empty() )
    {
        /* Frontend access type might change during SeriesImpl::read() to allow
...
void SeriesImpl::readGorVBased( bool do_init )
{
    auto & series = get();
    Parameter< Operation::OPEN_FILE > fOpen;
    ...
    case IterationEncoding::variableBased: {
        ...
        {
            index =
                series.iterations.getAttribute( "snapshot" ).get< uint64_t >();
        }
...
std::string SeriesImpl::iterationFilename( uint64_t i )
{
    auto & series = get();
    ...
    iteration << std::setw( series.m_filenamePadding ) << std::setfill( '0' )
              << i;
...
AdvanceStatus SeriesImpl::advance(
    AdvanceMode mode,
    ...
    Iteration & iteration )
{
    ...
    // We cannot call SeriesImpl::flush now, since the IO handler is still
...
namespace
{
    ...
    buildMatcher( std::string const & regexPattern )
    {
        ...
        return [ pattern ]( std::string const & filename ) -> Match {
            ...
            return {
                match, padding, match ? std::stoull( regexMatches[ 1 ] ) : 0 };
        };
    }
